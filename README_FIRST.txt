...
V22 provides three clearly separated experiences:
...
UPLOAD
1. Keep the prior V21 ZIP unchanged as the rollback package.
...
6. Open a private/incognito browser window and test:
   BHS: https://app.palonious.com/
   GHS: https://app.palonious.com/ghs
7. Confirm both school explorers open directly without an access-code screen and create one no-photo roadmap in each.
8. Select the Administrator Preview button, enter the private preview code, choose either fictional Grade 9 or Grade 11 student, then select a career and future age.
...
Administrator Preview does not accept, upload or process a real student photograph. Its two sample faces are bundled fictional images made for demonstration. Each live transformation still uses the configured image API and is therefore described under the provider's standard API data controls; V22 does not call that a Zero Data Retention workflow.
...
WHAT V22 ADDS
• BHS and GHS no-photo course explorers now open directly from their public URLs.
• The access code is requested only after an educator selects Administrator Preview.
• Successful access returns the educator to the requested BHS or GHS preview.
• The no-photo roadmap API remains available without the preview code.
